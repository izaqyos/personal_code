...
public:
    int trap(vector<int>& height) {
        ...
        do 
        {
            cout<<"list size: "<<height.size()<<endl;
            //trim leading & trailing zeros
            while (height[0] == 0) height.erase(height.begin());
            while (height[height.size()-1] == 0) height.erase(height.begin() +height.size()-1);
                
            
            
            if (height.empty()) 
            {
                cout<<"List is empty. breaking loop"<<endl;
                break;
            }
            li=-1,ri=-1;
