    ...
    bool isValidSudoku(vector<vector<char>>& board) {
        ...
        for (const auto & kv : chars) //in C++14 iterate key_pairs e.g. for (const auto & kv : chars) {kv.first;kv.second; 
        {
            ...
            for (int i=0; i<kv.second.size(); ++i)
            {
                ...
                {
                    if (kv.second[i].first == kv.second[j].first) return false; // row has 1-9 more than once
                    if (kv.second[i].second == kv.second[j].second) return false; // column has 1-9 more than once
