...
class Solution(object):
    ...
    def __init__(self):
        self.mPermCache = []
        for n in range(2,11):
            pGen = permGen(range(1,n))
            self.mPermCache.append(list(pGen) )
    ...
    def getPermutation(self, n, k):
        ...
        use space for o(1) speed. just keep 9 lists of all permutations 1-9! use permGen to precalculate  them on __init__
        """

        if (n < 1) or (n>9):
            return None

        return "".join(map(str, self.mPermCache[n-1][k-1]) )
        #if n == 0:
        #    return None
