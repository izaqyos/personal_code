...
#         self.next = next
class Solution:
    def sortListHeap(self, head: Optional[ListNode]) -> Optional[ListNode]:
        """
    ...
    #first partition starts at head, second at slow.next as slow ptr moves twice as slow as fast ptr he ends up in middle 
        ...
        while (fast.next) and (fast.next.next):
            ...
            fast = fast.next.next
        fast = slow.next 
        slow.next = None #break link between head tail (slow ptr) and right side partition list (slow.next which was stored in fast) , merge will recreate sorted list
