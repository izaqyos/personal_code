...
def main():
    #for i in genNumsToSquare(4):
    #    print "{0} \n ".format(i)

    M,N = 4,4
    sol = Solution()
    #testMatrix =  [ [ n for n in range(M)] for  line in range(N)                ]
    #sol.printMatrix(testMatrix)
    for n in range(5):
        print "Test matrix {0}X{0}".format(n,n)
        #sol.generateMatrix(n) 
        sol.printMatrix(sol.generateMatrix(n) )
        print "-"*120
