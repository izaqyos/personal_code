...
| 2026-06-28 | String to Integer (atoi) | 8 | Medium | PASS | 0 | O(n)/O(1) | atoi_v2.py, accumulate-then-clamp variant. 24/24 incl. exact bounds, one-past-bounds clamp, lone/double sign, leading zeros, beyond-64-bit. Key fix over v1: guard `if i < len(s): c = s[i] else: return ret` before reading the sign char — kills IndexError on "" / "   " (all-whitespace skips i to len). Three phases each touch chars once: skip ' ' / optional single sign / digit-accumulate stopping at first non-digit. Spec note: only ' ' is whitespace (not \t\n) — say it out loud, don't reach for .isspace(). Style nits: `c = s[i]` in digit loop is dead (only int(c) used → inline `int(s[i])`); stray `i< len`. Correct-in-Python ONLY because ints unbounded; C/Java would overflow the register BEFORE the end-clamp. TODO v3 = interview-grade overflow-DURING-accumulation: guard `ret > (INT_MAX - d)//10` before ret=ret*10+d. Alts: DFA/state-machine (4 states, LC official, the "make it extensible" answer); regex `\s*([+-]?\d+)` (usually banned — hides the index logic) |
| 2026-07-02 | String to Integer (atoi) (v3: pre-check) | 8 | Medium | PASS | 3 (layered, socratic) | O(n)/O(1) | atoi_v3.py, overflow-DURING-accumulation — the interview-grade variant. Guard BEFORE `ret = ret*10 + d`: pos `temp_ret > (INT_MAX - d)//10`, neg `temp_ret <= (INT_MIN + d)//10`. Three-bug path: (1) first attempt checked AFTER the multiply — still v2-in-disguise, only works b/c Python bigints; in C the register is already garbage before the compare. (2) algebra sign flip: next neg value is t*10 - d, moving d across gives INT_MIN **+** d (wrote -d → threshold too lenient → exactly-one-past "-2147483649" escaped while far-past cases got caught a digit late). (3) floor div on negatives rounds toward -inf, eating the .9 → strict < misses boundary by rounding width → need <=. The <= freebie: firing at exact INT_MIN returns INT_MIN = the right answer anyway (exact-min produced via clamp path, never accumulated); pos side has no such luxury — INT_MAX must survive accumulation, hence strict >. Diagnostic tell: far-past overflows caught but exactly-one-past escapes = guard firing one iteration late. Cleaner single-check form: absorb the |INT_MIN|=INT_MAX+1 asymmetry into a sign-picked limit (INT_MAX+1 when neg), guard magnitude `ret > (limit-d)//10` — temp_ret and neg floor-div both vanish. Alt: C-classic constant threshold `ret > INT_MAX//10 or (ret==INT_MAX//10 and d>7)`. Bonus perk over v2: early bail — "9"*10^6 stops after ~11 digits, no megadigit bigint. Nit: .isdigit() wider than ASCII ('²' passes, int() raises); strict check is '0'<=c<='9' |
