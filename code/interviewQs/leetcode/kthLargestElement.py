...
class Solution:
    ...
    def findKthLargest(self, nums: List[int], k: int) -> int:
        ...
        def quick_select(l,r,kth_index): #for kth smallest
            #print(f"got kth_index={kth_index}, l={l}, r={r}") 
            if l == r:
                return nums[l]
            pivot_index = partition(l,r)
            #print(f"partition result pivot_index={pivot_index}, pivot={nums[pivot_index]}") 
            #print(f"nums={nums}") 

            if pivot_index == kth_index:
                return nums[pivot_index]
            if pivot_index > kth_index: #if index is bigger than k than k is in the left part
...
fails. should be 1. returns None
"""
