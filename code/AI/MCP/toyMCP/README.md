...
There are multiple ways to test the ToyMCP server:

### Full End-to-End Testing

The most reliable way to test the entire application stack is using the provided `run_full_test.sh` script:

```bash
# Run the full test suite (recommended approach)
./run_full_test.sh
```

**What the script does:**
1. Stops and removes the existing database container and volumes
2. Starts a fresh PostgreSQL database container
3. Waits for the database to initialize
4. Starts the Node.js server in the background
5. Runs a series of API tests using `test_server.sh`
6. Shuts down the server and reports test results

**Prerequisites:**
- Docker must be installed and running
- bash shell environment
- curl and jq installed (`brew install curl jq` on macOS or `apt install curl jq` on Debian/Ubuntu)

This approach ensures that tests always run against a clean database state and properly configures all components of the system, avoiding common issues like port conflicts or stale database state.
...
3. **Manual API Testing:**
   The repository includes a `requests.http` file that can be used with REST client extensions in editors like VS Code for interactive API testing.

**Note:** When running tests manually, be aware that some tests may fail if preconditions (like database state) are not properly set up.
