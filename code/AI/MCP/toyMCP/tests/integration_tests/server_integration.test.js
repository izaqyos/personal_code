...
beforeAll(async () => {
    // Start the server
    // server = app.listen(SOME_TEST_PORT); // Ensure your app can be started like this and port is unique for tests

    // Ensure the test user exists and we can log in.
    // It's often better to seed a test user directly into the DB for integration tests
    // or have a dedicated test user creation endpoint that isn't part of the main auth flow.

    try {
        // Attempt to log in
        const response = await request(app)
            .post('/auth/login') // Make sure this is your correct login path
            .send({
                username: 'testuser', // Use a known test user
                password: 'password123' // Use its correct password
            });

        if (response.status === 200 && response.body.token) {
            authToken = response.body.token;
            console.log('Successfully obtained auth token for integration tests.');
        } else {
            console.error('Failed to obtain auth token in beforeAll. Status:', response.status, 'Body:', response.body);
            // Attempt to register the user if login fails, assuming a registration endpoint exists
            // This is a fallback, ideally the test user should pre-exist or be seeded.
            try {
                console.log('Attempting to register testuser...');
                const regResponse = await request(app)
                    .post('/auth/register') // Make sure this is your correct registration path
                    .send({
                        username: 'testuser',
                        password: 'password123',
                        // email: 'testuser@example.com' // if required
                    });
                if (regResponse.status === 201 || regResponse.status === 200) { // Check for successful registration
                    console.log('Test user registered. Retrying login...');
                    ...
                    if (loginRetryResponse.status === 200 && loginRetryResponse.body.token) {
                        authToken = loginRetryResponse.body.token;
                        console.log('Successfully obtained auth token after registration.');
                    } else {
                         console.error('Failed to login even after attempting registration. Status:', loginRetryResponse.status, 'Body:', loginRetryResponse.body);
                    }
                } else {
                    console.error('Failed to register test user. Status:', regResponse.status, 'Body:', regResponse.body);
                }
            } catch (regError) {
                console.error('Error during test user registration attempt:', regError.message);
            }
        }
    } catch (error) {
        console.error('Error in beforeAll while trying to authenticate:', error.message);
    }
    // It's crucial to throw an error here if authToken is still not obtained,
    // otherwise, tests will run without auth and give misleading results.
    if (!authToken) {
        // This line was in your log, so it's good to keep it to fail fast.
        throw new Error('Authentication token not obtained in beforeAll hook. Further tests will likely fail.');
    }
}, 30000); // Increase timeout for beforeAll if server startup/DB ops take time
...
afterAll(async () => {
    ...
    // }
    // await pool.end(); // Close db connection if you opened one directly
});
