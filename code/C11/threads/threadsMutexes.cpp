...
class RCDemo
{
	...
	{
		//We can manually lock and unlock - but - what happens if we have uncontolled jump from critical section
		// say an exception? - Well mutex will stay locked preventing all other threads from CS
		// Solution use RAAI lock_guard (RAAI - Resource Acquistion Is Initialization - so lock_guard CTOR locks and DTOR unlocks
		//mut.lock()
