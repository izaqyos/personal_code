...
int main() {
    std::string first;
    std::getline(std::cin, first); //read line from cin to first
    std::istringstream iss(first); // init istringstream for tokenizing first
    std::vector<int> tokens1;
    std::copy(std::istream_iterator<int> (iss), //init iter to start of iss
                 std::istream_iterator<int>(),//init iter to end of sequence
                 std::back_inserter(tokens1)); //copy to tokens1 by push_back
    //so basicaly this parses n,q input line
    //I'd rather do cin>>n>>q; lol
