...
void rotate(std::vector<int> &a, int k)
{
    ...
    */
    std::cout << "Rotating array by " << k << " steps. Original array: ";
    util::print_vector(a, std::cout);
    if (a.empty())
        return;
    unsigned int n = a.size();
    std::cout<<"Normalized k mod n: " << k << "\n";
    if (k == 0)
        return;
    if (k < 0) { // left rotate by |k| is right rotate by n - |k|, I can't use -k % n b/c of negative modulo results (specifically in C++, array size is unsigned int so 
        // it causes auto conversion of -k to a large positive number, say -1 would become 2^64-1 on a 64-bit system)
        k = -k;
        k = k % n;
        k = n - k; 
    } else {
    ...
    std::cout << "Normalized k: " << k << "\n";

    std::reverse(a.begin(), a.end());
    std::cout << "After reversing entire array: ";
    util::print_vector(a, std::cout);
    std::reverse(a.begin(), a.begin() + k);
    std::cout << "After reversing array 0..k-1: ";
    util::print_vector(a, std::cout);
    std::reverse(a.begin() + k, a.end());
    std::cout << "After reversing array k..n: ";
    util::print_vector(a, std::cout);
}
