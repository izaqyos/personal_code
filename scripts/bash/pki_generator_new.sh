...
function help ()
 {
  echo "Usage is: \"$0 [ Options ] \" "
	echo " Options are : "
	echo "   -Cm, --CA_mode [Options] allowed values are \"noCA\",\"CA\",\"policyCa\""
   ...
   echo "   -issuingCaexp, --policyCa_experation [Options] allowed values are [0-99999...]"
...
# Example Usage
#  ./pki_generator_new.sh -Cm CA -Noc 10 -Norc 5 -up user -mp machine -dp cisco.com  -CAexp 365 -policyCaexp 365 -issuingCaexp 365 -SERexp 365 -CLTexp 365 -hash sha2 -keysz 1024 -RndSn
...
echo "Whould you like to [r]emove directory or [a]bort execution"
...
echo "organizationName         = Cisco">> ca.config
echo "organizationalUnitName         = ACS Division">> ca.config
echo "commonName         = $cfp">> ca.config
echo "emailAddress     = $cfp@$domainpatern">> ca.config
echo "">> ca.config
echo "[ rootca_cert ]">> ca.config
echo "# Following section discribes extentions which are being added to the rootCA">> ca.config
echo "">> ca.config
...
openssl req -new -x509  -passout pass:$passwd -keyout ca/cakey.pem -out ca/cacert.cer -days $CAExp -passin pass:$passwd -config ./ca.config -set_serial $rndSerial
...
echo "organizationalUnitName = ACS Division" >> server-req-cert.config
#echo "commonName     = ACSServer" >> server-req-cert.config
echo "commonName     = ACSServer" >> server-req-cert.config
echo "emailAddress     = ACSServer@cisco.com" >> server-req-cert.config
...
echo "nsComment               = \"Server certificate generated using OpenSSL for ACS\"" >> server.config
